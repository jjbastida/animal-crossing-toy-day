...
const selectionButton = (selected: boolean, index: number) => css`
  ...
  border-radius: 999px;
  background: ${selected ? `${playerColors[index]}20` : `url("data:image/svg+xml,${containerSvg("#EFEADD", "999px")}")`};
  border: none;
  cursor: pointer;
  transition: all 0.2s;

  &:hover {
    background: ${selected ? `${playerColors[index]}20` : `url("data:image/svg+xml,${containerSvg(playerColors[index], "999px")}")`};

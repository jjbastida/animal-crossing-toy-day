...
function handleFruitAbilities(players: Player[]): Player[] {
  return players.map(player => {
    const updatedPlayer = { ...player, fruitValue: 50, bells: 500, inventory: [] as Item[] };
    
    switch (player.fruit) {
      case 'apple':
        updatedPlayer.bells = 1500;
        break;
      case 'pear': {
        updatedPlayer.inventory = [...updatedPlayer.inventory, {
          name: itemIcons['nook-miles-ticket'].name,
          description: 'A ticket that can be used to get a free item from the shop.',
...
function CharacterCreationPage(): React.ReactNode {
  ...
  function modifyPlayer(playerId: number, key: string, value: string | number | FruitType | AvatarType) {
  ...
  return (
    <div css={container}>
      <Typography variant="display" size="3xl" css={styles.title} style={modalOpen ? { opacity: '0.5' } : {}}>Build your villager!</Typography>
      <CharacterCustomization modifyPlayer={modifyPlayer} modalOpen={modalOpen} setModalOpen={setModalOpen} />

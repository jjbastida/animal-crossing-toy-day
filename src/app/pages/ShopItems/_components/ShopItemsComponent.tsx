...
import { container, title, resourceCount, itemsGrid, itemCard, itemName, itemCost, getButtonStyle, itemsList, item, titleSmall } from './ShopItemsComponent.styles.ts';
    ...
    <div css={container}>
      ...
      {items.length > 0 && (
        <>
          ...
          <div css={itemsList}>
            {items.map((ownedItem) => (
              <div css={item} key={ownedItem.id}>{ownedItem.name}</div>
            ))}
          </div>
        </>
      )}
